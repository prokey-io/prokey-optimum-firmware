...
bool protectChangePin(bool removal) {
  ...
  if (config_hasPin()) {
    ...
    if(PinNumberEnter(ButtonRequestType_ButtonRequest_EnterPinOnDevice, true, "CURRENT Pin", pin) != PinNumberOk)
    {
      fsm_sendFailure(FailureType_Failure_PinCancelled, NULL);
      return false;
    }
  ...
  if (!removal) {
    if (PinNumberEnter(ButtonRequestType_ButtonRequest_EnterNewPinOnDevice, false, "Enter NEW Pin", pin) != PinNumberOk) {
    ...
    if (PinNumberEnter(ButtonRequestType_ButtonRequest_ReEnterNewPinOnDevice, true, "RE-ENTER NEW Pin", pin) != PinNumberOk) {
      memzero(old_pin, sizeof(old_pin));
