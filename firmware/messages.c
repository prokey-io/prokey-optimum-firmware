...
void MsgWriteOutRaw( const unsigned short msg_id, const unsigned char* data, const unsigned int len )
{
	msg_out_append('#');
	msg_out_append('#');
	msg_out_append((msg_id >> 8) & 0xFF);
	msg_out_append(msg_id & 0xFF);
	msg_out_append((len >> 24) & 0xFF);
	msg_out_append((len >> 16) & 0xFF);
	msg_out_append((len >> 8) & 0xFF);
	msg_out_append(len & 0xFF);
	for( unsigned int i=0; i<len; i++ ){
		msg_out_append(data[i]);
	}
	msg_out_pad();
}
...
void msg_read_common(char type, const uint8_t *buf, uint32_t len) {
  ...
  if (read_state == READSTATE_IDLE) {
    ...
    if( (msg_id & 0xC000) == 0xC000 )
		{
			//! TO DO: ProkeyMessageFields should return fields
			//! Now we will handle simple commands without protobuf
			//fields = ProkeyMessageFields(type, 'i', msg_id);
		} else {
  ...
  if (msg_pos >= msg_size) {
    // Check prokey mutual-authenticate commands
    if( msg_id & 0xC000 ) {
			if( msg_id == 0xFFF0 ) {
				//! Device Reset
				DeviceReset(true);
				return;
			} else if( msg_id == 0xFFF1 ) {
				MsgAuthenticate(msg_in);
			}
		}
		else
